...
> [!NOTE]
> Although, there is reasonable evidence that the brain may be using movements instead of locations, it is unclear whether there are any computational benefits to doing this. As such, the `improved-metric` field is left open. A first step to tackling this task would be to think through any functional implications of this change. In the meantime, we include this task because choosing the write assumptions about a system is key to developing long-term, successful solutions. Using displacements rather than locations would have impacts both for how voting and motor actions are implemented. Continuing to use locations, and diverging from a more brain-like approach, may therefore lead us into a "local minimum" style solution for many elements in Monty.
